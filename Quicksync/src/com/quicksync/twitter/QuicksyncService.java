...
import com.quicksync.twitter.IAndTweetService;
import com.quicksync.twitter.IAndTweetServiceCallback;
import com.quicksync.twitter.R;
import com.quicksync.twitter.TwitterUser.CredentialsVerified;
import com.quicksync.twitter.appwidget.QuicksyncAppWidgetProvider;
import com.quicksync.twitter.data.QuicksyncDatabase;
import com.quicksync.twitter.data.FriendTimeline;
import com.quicksync.twitter.net.ConnectionAuthenticationException;
import com.quicksync.twitter.net.ConnectionException;
import com.quicksync.twitter.net.ConnectionUnavailableException;
import com.quicksync.twitter.util.I18n;
 ...
 */
public class QuicksyncService extends Service {
	private static final String TAG = QuicksyncService.class.getSimpleName();
	/**
    ...
    private static final String packageName = QuicksyncService.class.getPackage().getName();
    ...
    private void notifyNewTweets(int numTweets, int msgType) {
        ...
        // com.quicksync.twitter.TimelineActivity.onOptionsItemSelected
        switch (msgType) {
            case NOTIFY_REPLIES:
                aMessage = I18n
                ...
                intent.putExtra(QuicksyncService.EXTRA_TIMELINE_TYPE,
                        QuicksyncDatabase.Tweets.TIMELINE_TYPE_MENTIONS);
            ...
            case NOTIFY_DIRECT_MESSAGE:
                ...
                intent.putExtra(QuicksyncService.EXTRA_TIMELINE_TYPE,
                        QuicksyncDatabase.Tweets.TIMELINE_TYPE_MESSAGES);
                contentIntent = PendingIntent.getActivity(getApplicationContext(), numTweets,
                        intent, 0);
                break;
            ...
            default:
                ...
                intent = new Intent(getApplicationContext(), TweetListActivity.class);
                intent.putExtra(QuicksyncService.EXTRA_TIMELINE_TYPE,
                        QuicksyncDatabase.Tweets.TIMELINE_TYPE_FRIENDS);
                contentIntent = PendingIntent.getActivity(getApplicationContext(), numTweets,
                        intent, 0);
                break;
        }
	 ...
	 * @see QuicksyncAppWidgetProvider
	 */
	...
	protected Runnable mLoadTimeline = new Runnable() {
		public void run() {
	        ...
	        if (TwitterUser.getTwitterUser(QuicksyncService.this.getApplicationContext()).getCredentialsVerified() == CredentialsVerified.SUCCEEDED) {
	            // Only if User was authenticated already
    			try {
    	            FriendTimeline fl = new FriendTimeline(QuicksyncService.this.getApplicationContext(), QuicksyncDatabase.Tweets.TIMELINE_TYPE_MENTIONS);
    				fl.loadTimeline();
    				aReplyCount = fl.replyCount();
                    
    				fl = new FriendTimeline(QuicksyncService.this.getApplicationContext(), QuicksyncDatabase.Tweets.TIMELINE_TYPE_FRIENDS);
    				fl.loadTimeline();
	...
	protected Runnable mLoadMessages = new Runnable() {
		public void run() {
            ...
            if (TwitterUser.getTwitterUser(QuicksyncService.this.getApplicationContext()).getCredentialsVerified() == CredentialsVerified.SUCCEEDED) {
                // Only if User was authenticated already
    			try {
    	            FriendTimeline fl = new FriendTimeline(getApplicationContext(), QuicksyncDatabase.Tweets.TIMELINE_TYPE_MESSAGES);
    				fl.loadTimeline();
    ...
    /**
     * Utility method to send the Intent to the {@link QuicksyncService} that
    ...
    public static void startAutomaticUpdates(Context context) {
        ...
        serviceIntent.putExtra(QuicksyncService.EXTRA_MSGTYPE, QuicksyncService.ACTION_START_ALARM);
    ...
    public static void stopAutomaticUpdates(Context context) {
        Intent serviceIntent = new Intent(IAndTweetService.class.getName());
        serviceIntent.putExtra(QuicksyncService.EXTRA_MSGTYPE, QuicksyncService.ACTION_STOP_ALARM);
        context.startService(serviceIntent);
    }
    ...
    public static void restartAutomaticUpdates(Context context) {
        Intent serviceIntent = new Intent(IAndTweetService.class.getName());
        serviceIntent.putExtra(QuicksyncService.EXTRA_MSGTYPE, QuicksyncService.ACTION_RESTART_ALARM);
        context.startService(serviceIntent);
    }
    ...
    public static int d(String tag, String msg) {
        int i = 0;
        if (Log.isLoggable(QuicksyncService.APPTAG, Log.DEBUG)) {
            i = Log.d(TAG, msg);
        }
        return i;
    }
