...
import com.quicksync.twitter.data.QuicksyncDatabase.DirectMessages;
 ...
 * @author torgny.bjers
 */
public class QuicksyncProvider extends ContentProvider {

    private static final String TAG = QuicksyncProvider.class.getSimpleName();
    ...
    public Cursor query(Uri uri, String[] projection, String selection, String[] selectionArgs,
        ...
        Cursor c = null;
        boolean logQuery = Log.isLoggable(QuicksyncService.APPTAG, Log.VERBOSE);
        try {
            if (sql.length() > 0) {
    ...
    static {
        ...
        sUriMatcher.addURI(QuicksyncDatabase.AUTHORITY, TWEETS_TABLE_NAME, TWEETS);
        sUriMatcher.addURI(QuicksyncDatabase.AUTHORITY, TWEETS_TABLE_NAME + "/#", TWEET_ID);
        sUriMatcher.addURI(QuicksyncDatabase.AUTHORITY, TWEETS_TABLE_NAME + "/search/*",
                TWEET_SEARCH);
        sUriMatcher.addURI(QuicksyncDatabase.AUTHORITY, TWEETS_TABLE_NAME + "/count", TWEETS_COUNT);

        sUriMatcher.addURI(QuicksyncDatabase.AUTHORITY, USERS_TABLE_NAME, USERS);
        sUriMatcher.addURI(QuicksyncDatabase.AUTHORITY, USERS_TABLE_NAME + "/#", USER_ID);

        sUriMatcher.addURI(QuicksyncDatabase.AUTHORITY, DIRECTMESSAGES_TABLE_NAME, DIRECTMESSAGES);
        sUriMatcher.addURI(QuicksyncDatabase.AUTHORITY, DIRECTMESSAGES_TABLE_NAME + "/#",
                DIRECTMESSAGE_ID);
        sUriMatcher.addURI(QuicksyncDatabase.AUTHORITY, DIRECTMESSAGES_TABLE_NAME + "/count",
