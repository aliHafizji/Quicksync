 ...
 */
package com.quicksync.twitter.appwidget;
...
import com.quicksync.twitter.MessageListActivity;
import com.quicksync.twitter.TweetListActivity;
import com.quicksync.twitter.data.QuicksyncDatabase;
import com.quicksync.twitter.util.I18n;
import com.quicksync.twitter.util.RelativeTime;
 ...
 */
public class QuicksyncAppWidgetProvider extends AppWidgetProvider {
	// log tag
	private static final String TAG = QuicksyncAppWidgetProvider.class
			.getSimpleName();

	private int msgType = QuicksyncService.NOTIFY_INVALID;
	...
	public void onReceive(Context context, Intent intent) {
		...
		if (QuicksyncService.ACTION_APPWIDGET_UPDATE.equals(action)) {
            if (Log.isLoggable(QuicksyncService.APPTAG, Log.VERBOSE)) {
                Log.v(TAG, "inst=" + instanceId + "; Intent from AndTweetService received!");
            }
			Bundle extras = intent.getExtras();
			if (extras != null) {
				msgType = extras.getInt(QuicksyncService.EXTRA_MSGTYPE);
				numSomethingReceived = extras
						.getInt(QuicksyncService.EXTRA_NUMTWEETS);
	...
	public void onDeleted(Context context, int[] appWidgetIds) {
		...
		final int N = appWidgetIds.length;
		for (int i = 0; i < N; i++) {
			new QuicksyncAppWidgetData(context, appWidgetIds[i]).delete();
		}
	}
	...
	void updateAppWidget(Context context, AppWidgetManager appWidgetManager,
		...
		try {
			...
			QuicksyncAppWidgetData data;		
			synchronized(xlock) {
				data = new QuicksyncAppWidgetData(context,
						appWidgetId);
				data.load();

		        if (QuicksyncService.updateWidgetsOnEveryUpdate || (numSomethingReceived != 0)) {
                    data.changed = true;
		        }
				// Calculate new values
				switch (msgType) {
				case QuicksyncService.NOTIFY_REPLIES:
					data.numMentions += numSomethingReceived;
					break;
		
				case QuicksyncService.NOTIFY_DIRECT_MESSAGE:
					data.numMessages += numSomethingReceived;
					break;
		
				case QuicksyncService.NOTIFY_TIMELINE:
					data.numTweets += numSomethingReceived;
					break;
		
				case QuicksyncService.NOTIFY_CLEAR:
					data.clear();
					break;
			...
			Intent intent;
			int timeLineType = QuicksyncDatabase.Tweets.TIMELINE_TYPE_FRIENDS;
			if (data.numMessages > 0) {
	            intent = new Intent(context, MessageListActivity.class);
			    timeLineType = QuicksyncDatabase.Tweets.TIMELINE_TYPE_MESSAGES;
			} else {
	            intent = new Intent(context, TweetListActivity.class);
			    if (data.numMentions > 0) {
	                timeLineType = QuicksyncDatabase.Tweets.TIMELINE_TYPE_MENTIONS;
			    }
			}
            ...
            intent.setData((android.net.Uri.parse(QuicksyncDatabase.Tweets.CONTENT_URI.toString() + "#" + android.os.SystemClock.elapsedRealtime())));
            ...
            if ( !Ok || Log.isLoggable(QuicksyncService.APPTAG, Log.VERBOSE)) {
