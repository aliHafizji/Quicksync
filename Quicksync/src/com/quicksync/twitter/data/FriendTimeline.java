...
import com.quicksync.twitter.QuicksyncService;
import com.quicksync.twitter.PreferencesActivity;
import com.quicksync.twitter.TwitterUser;
import com.quicksync.twitter.TwitterUser.CredentialsVerified;
import com.quicksync.twitter.data.QuicksyncDatabase.Tweets;
...
import com.quicksync.twitter.util.SelectionAndArgs;
...
public class FriendTimeline {
    ...
    public FriendTimeline(Context context, int timelineType) {
        ...
        switch (mTimelineType) {
            case QuicksyncDatabase.Tweets.TIMELINE_TYPE_FRIENDS:
            case QuicksyncDatabase.Tweets.TIMELINE_TYPE_MENTIONS:
                mContentUri = QuicksyncDatabase.Tweets.CONTENT_URI;
                mContentCountUri = QuicksyncDatabase.Tweets.CONTENT_COUNT_URI;
                break;
            case QuicksyncDatabase.Tweets.TIMELINE_TYPE_MESSAGES:
                mContentUri = QuicksyncDatabase.DirectMessages.CONTENT_URI;
                mContentCountUri = QuicksyncDatabase.DirectMessages.CONTENT_COUNT_URI;
                break;
        }
    ...
    public Uri insertFromJSONObject(JSONObject jo) throws JSONException, SQLiteConstraintException {
        ...
        switch (mTimelineType) {
            ...
            case QuicksyncDatabase.Tweets.TIMELINE_TYPE_MENTIONS:
                ...
                values.put(QuicksyncDatabase.Tweets._ID, lTweetId.toString());
                values.put(QuicksyncDatabase.Tweets.AUTHOR_ID, user.getString("screen_name"));

                values.put(QuicksyncDatabase.Tweets.MESSAGE, message);
                values.put(QuicksyncDatabase.Tweets.SOURCE, jo.getString("source"));
                values.put(QuicksyncDatabase.Tweets.TWEET_TYPE, mTimelineType);
                values.put(QuicksyncDatabase.Tweets.IN_REPLY_TO_STATUS_ID, jo
                        .getString("in_reply_to_status_id"));
                values.put(QuicksyncDatabase.Tweets.IN_REPLY_TO_AUTHOR_ID, jo
                        .getString("in_reply_to_screen_name"));
                values.put(QuicksyncDatabase.Tweets.FAVORITED, jo.getBoolean("favorited") ? 1 : 0);
                break;
            case QuicksyncDatabase.Tweets.TIMELINE_TYPE_MESSAGES:
                values.put(QuicksyncDatabase.DirectMessages._ID, lTweetId.toString());
                values.put(QuicksyncDatabase.DirectMessages.AUTHOR_ID, jo
                        .getString("sender_screen_name"));
                values.put(QuicksyncDatabase.DirectMessages.MESSAGE, message);
                break;
        }
    ...
    public int pruneOldRecords() {
        ...
        if (maxDays > 0) {
            ...
            sa.addSelection(QuicksyncDatabase.Tweets.SENT_DATE + " <  ?", new String[] {
                String.valueOf(sinceTimestamp)
            });

            if (mTimelineType != QuicksyncDatabase.Tweets.TIMELINE_TYPE_MESSAGES) {
                // Don't delete Favorites!
                sa.addSelection(QuicksyncDatabase.Tweets.FAVORITED + " = ?", new String[] {
                    "0"
                });
            }
            ...
            try {
                ...
                if (nToDeleteSize > 0) {
                    ...
                    if (sinceTimestampSize > 0) {
                        ...
                        if (mTimelineType != QuicksyncDatabase.Tweets.TIMELINE_TYPE_MESSAGES) {
                            sa.addSelection(QuicksyncDatabase.Tweets.FAVORITED + " = ?",
                                    new String[] {
                                        "0"
                                    });
                        }
    ...
    public int destroyStatus(long statusId) {
        return mContentResolver.delete(mContentUri, QuicksyncDatabase.Tweets._ID + " = " + statusId,
                null);
    }
}
